...
For the monojet signal, the generation is performed using the mediator mass for the renormalisation and factorisation scales.
...
algorithm~\cite{CMS-PAS-PFT-09-001,CMS-PAS-PFT-10-001}, which optimally combines information from various components 
...
to ensure a trigger efficiency greater than 99\% for selected events. 
...
sample, produced at next-to-leading order (NLO) with {\sc Powheg1.0} ~\cite{Nason:2004rx,Frixione:2007vw,powheg,Alioli:2010xd,Alioli:2009je}, 
and a set of diboson samples,
...
$\pt>30$\GeV and $|\eta|<2.5$ are selected, providing the azimuthal angle between it and the highest \pt jet with $|\eta|<2$  
...
$p_{\mathrm{T}}$ photon or a single muon consistent with being from a leptonic W boson decay. This 
...
the control regions is used to estimate the distribution of $\ETm$ expected from the \Zvvjets~and \Wlvjets~
...
observed between data and simulation in events selected by inverting the requirement on $\Detla(\ETm,\textrm{j})$.
...
after the fit in the control regions, are given in 
Tables~\ref{tab:bkgmonojet},~\ref{tab:bkgresolved} and~\ref{tab:bkgboosted} for each of the three signal regions,. 
...
in Fig.~\ref{fig:post_fit_plots} for each of the three categories.   Agreement between the expected SM
...
The translations are obtained following the procedures outlined in Ref.~\cite{Malik:2014ggr} 
...
light-quark couplings~\cite{Harris:2015kda}.  
...
In the axial vector model, the limits from this analysis are stronger than the DD limits for DM masses up to $m_{\mathrm{DM}}=300$ \GeV. 
...
all of these regions are excluded by this analysis. The limits from this analysis exceed those from Fermi LAT when the DM mass is below 100\GeV.
