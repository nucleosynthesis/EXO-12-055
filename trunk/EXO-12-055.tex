...
proton-proton (pp) collisions at the CERN Large Hadron Collider (LHC) at a
...
theories with nonuniversal DM couplings~\cite{IVDM}.  The interpretation of
results from these and other DM searches at the LHC have generally utilised
...
identify the individual jets from moderately lorentz-boosted vector bosons. The
...
event. The signal extraction is performed by considering a maximum
likelhiood over to the $\ETm$ distribution in each event category, providing improved
sensitivity compared to the previous CMS monojet
analysis~\cite{monojet1} based on the same dataset. 
...
multipurpose apparatus designed to study high-transverse momentum($\pt$) physics processes in
...
Pseudoscalar mediated interactions are velocity suppressed in
DM-nucleon interactions leading to limited sensitivity of DD to this type of mediators~\cite{Haisch:2012kf}.  
An extension to the scalar and pseudoscalar interactions can
be achieved by allowing the scalar and pseudoscalar mediated interactions to undergo
...
interactions occurs predominantly through gluon-fusion via a top quark loop 
(Fig. ~\ref{fig:monoXfeyn}(a)).  When EWSB is present in the model, mono-V
...
SM particles and DM is taken as unity ($g_{\mathrm{SM}}=g_{\mathrm{DM}}=1$). 
For the scalar and pseudoscalar models, $g_{\mathrm{SM}}$
denotes a Yukawa-type coupling to standard model particles. For all models
...
All signal models are generated at leading order.  
PYTHIA~6 .4.26~\cite{Sjostrand:2006za} is used for parton showers and
GEANT4~\cite{geant4} for simulation of the CMS detector response.  
For the monojet signal, the generation is performed using the mediator
mass for both the renormalisation and factorisation scale during the
generation  and for the scale passed to the parton shower. 
The impact of the parton shower scale choice was found to be increase
from 30-80\% in the expected signal yield in the relevant kinematic region 
for mediator masses above 400 GeV when compared with using the boson\pt for the shower scale. 
For scalar and pseudoscalar mediated DM production, MCFM takes into
account the finite top quark mass for both the inclusive and
differential cross section. NNPDF3.0 used to
...
\caption{Diagrams for production of DM via mediator (X) in the cases
  of a scalar mediator providing (a) monojet and (b) mono-V
  signatures. (c) Diagram for production of DM via a vector or axial vector mediator.\label{fig:monoXfeyn}} \end{figure}
...
algorithm~\cite{CMS-PAS-PFT-09-001,CMS-PAS-PFT-10-001}, together with a jet of \pt$>80$ \gev and
$|\eta|<2.6$. The lowest threshold on \ETm for event selection is set to 200 GeV 
...
the anti-$k_{\textrm{T}}$ algorithm~\cite{Cacciari:2008gp} with
distance parameter of 0.5 (AK5 jet), and the Cambridge--Aachen algorithm~\cite{cajets} with
distance parameter 0.8 (CA8). The leading jet is further required to
be pass standard CMS identification criteria~\cite{jec}. The jets
...
momentum, both its hadronic decay products are captured within a single
reconstructed CA8 ``fat'' jet.  Events in the V-boosted category are
...
in Refs.~\cite{Thaler:2010tr,Thaler:2011gf}, which identifies jets with a two-subjet
...
$\Delta\phi(\mathrm{AK5,CA8})$, is smaller than 2 radians,~the event is selected. Events
...
its hadronic decay to be fully contained in a single reconstructed CA8 jet, a
selection that looks for decays into a pair of AK5 jets is applied to recover
the event.  The selection requires that each jet
...
second AK5 jet close to the leading one ($\Delta\phi({\rm j_1,j_2}) < 2$) with
...
combined. The dashed red line shows the expected distribution assuming vector-mediated DM production with $m_{\mathrm{DM}}=10$ GeV and $m_{\mathrm{MED}}=1$ TeV.  
...
orthogonal to the signal region, with negligible contributions expected from signal, to
simultaneously constrain the predictions of the SM background. A
binned likelihood fit is performed in the $\ETm$ range 250--1000 \gev (200--1000 \gev) 
for the V-tagged (monojet) events, where the binning is chosen to
...
using the same selection as for the signal region, but removing the
muon veto. Instead, a pair of oppositely charged 
isolated muons are required with opposite charge, $\pt^{\mu_{1},\mu_{2}}>20,10$ GeV and an 
...
is the azimuthal angle between the muon and $\ETm$ vector in the transverse plane, must satisfy $50<m_{\mathrm{T}}<100$ \gev.
...
As the branching fraction of \Zmm~is approximately six times smaller than that
...
use of the binned likelihood fit vie equation\ref{eqn:candclh}, simultaneously across all bins in the three control regions.
...
uncertainties. The scale uncertainties for both the renormalisation
and factorisation scale are treated as partially correlated between
process($\gamma/W/Z$) with a
20\% decorrelated component designed to match the maximum scale
uncertainty on each individual process.  Electroweak corrections to the ratio are not accounted for in
...
for the top quark and diboson background normalisations~\cite{tagkey2015250} and
...
the b-jet veto in the V-resolved category. Systematic uncertainties of 7\%, 10\%
and 50\% are included for the top quark~\cite{Khachatryan:2015oqa},
...
\begin{sidewaystable}[htbp]
  \begin{center}
    \caption{Expected event yields of the standard model processes and
      their uncertainties per bin for the monojet category signal
      region after applying all corrections to the MC simulation.
      \label{tab:bkgmonojet}}
	\scriptsize
    \begin{tabular}{l|c|c|c|c|c|c|c|c}
	\hline
	${\rm E}_{\rm T}^{\rm miss} bin (GeV)$   & Observed &  Total Bkg & Z($\rightarrow ll$)+jets & Top~quark & QCD & W($\rightarrow l\nu$)+jets & Dibosons &  Z($\rightarrow \nu\nu$)+jets \\

\hline 
      200 - 210 & 17547  &  18330$\pm$600  &     59.8$\pm$1.5  &    132$\pm$11  &    480$\pm$220  &   6770$\pm$320  &    134$\pm$14  &  10740$\pm$270  \\ 

      210 - 220 & 14303  &  14500$\pm$610  &     44.9$\pm$1.2  &    104$\pm$13  &      6.5$\pm$3.0  &   4990$\pm$240  &    112$\pm$11  &   9230$\pm$230  \\ 

      220 - 230 & 11343  &  11370$\pm$400  &     31.5$\pm$0.8  &     82.1$\pm$7.3  &      4.4$\pm$2.0  &   3830$\pm$170  &     95.1$\pm$9.6  &   7320$\pm$190  \\ 

      230 - 240 & 8961  &   8940$\pm$400  &     26.1$\pm$0.7  &     62.0$\pm$5.8  &     21.3$\pm$9.6  &   3020$\pm$160  &     77.9$\pm$8.6  &   5730$\pm$170  \\ 

      240 - 250 & 6920  &   7290$\pm$330  &     18.6$\pm$0.5  &     46.6$\pm$4.4  &     12.0$\pm$5.4  &   2470$\pm$140  &     61.0$\pm$6.1  &   4680$\pm$150  \\ 

      250 - 260 & 5582  &   5670$\pm$370  &     14.3$\pm$0.4  &     34.2$\pm$3.7  &      5.6$\pm$2.5  &   1860$\pm$120  &     50.1$\pm$4.9  &   3700$\pm$140  \\ 

      260 - 270 & 4517  &   4950$\pm$320  &     10.2$\pm$0.3  &     27.7$\pm$2.3  &      $<$1  &   1580$\pm$110  &     39.7$\pm$4.2  &   3287$\pm$130  \\ 

      270 - 280 & 3693  &   3740$\pm$160  &      6.3$\pm$0.2  &     25.0$\pm$3.1  &      2.1$\pm$1.0  &   1101$\pm$71  &     33.5$\pm$3.4  &   2568$\pm$110  \\ 

      450 - 500 & 16  &      8.5$\pm$3.6  &      0.0$\pm$0.0  &      1.0$\pm$0.4  &      $<$1  &      1.0$\pm$2.6  &      0.4$\pm$0.1  &      6.1$\pm$2.7  \\ 

      500 - 1000 & 17  &     11.6$\pm$3.5  &      0.0$\pm$0.0  &      0.3$\pm$0.2  &      1.3$\pm$0.6  &      2.6$\pm$1.7  &      0.5$\pm$0.0  &      6.9$\pm$3.0  \\ 
...
\bigskip
\bigskip
    \caption{Expected event yields of the standard model processes and their
      uncertainties per bin for the V-boosted category signal region
      after applying all corrections to the MC simulation. QCD background is
      found to be negligable.
      \label{tab:bkgboosted}}
	\scriptsize
    \begin{tabular}{l|c|c|c|c|c|c|c|c}
	\hline
	${\rm E}_{\rm T}^{\rm miss} bin (GeV)$ & Observed  & Total Bkg  & Z($\rightarrow ll$)+jets & Top~quark & W($\rightarrow l\nu$)+jets & Dibosons &  Z($\rightarrow \nu\nu$)+jets \\
\hline 
     250 - 300 & 1073  &   1103$\pm$63  &      2.5$\pm$0.0  &     35.4$\pm$3.7  &    279$\pm$33  &    103$\pm$15  &    683$\pm$40  \\ 

      300 - 350 & 453  &    446$\pm$34  &      0.7$\pm$0.0  &     12.7$\pm$1.3  &    114$\pm$20  &     47$\pm$6.9  &    271$\pm$23  \\ 

      350 - 400 & 160  &    184$\pm$18  &      0.2$\pm$0.0  &      5.6$\pm$1.0  &     38.3$\pm$8.7  &     22.2$\pm$3.3  &    117$\pm$13  \\ 

      400 - 450 & 81  &     72$\pm$29  &      0.0$\pm$0.0  &      1.5$\pm$0.8  &      9.8$\pm$3.4  &     11.0$\pm$1.8  &     49.7$\pm$7.3  \\ 

      450 - 500 & 30  &     44.3$\pm$6.6  &      0.0$\pm$0.0  &      0.5$\pm$0.1  &      5.0$\pm$2.6  &      7.4$\pm$1.1  &     31.2$\pm$6.1  \\ 

      500 - 1000 & 39  &     54.3$\pm$8.5  &      0.0$\pm$0.0  &      0.2$\pm$0.0  &      6.4$\pm$3.4  &      7.8$\pm$1.1  &     39.8$\pm$7.8  \\ 
\hline 
    \end{tabular}
  \end{center}
\end{sidewaystable}
...
axial vector, scalar and pseudoscalar mediator models.  The expected
90\% CL upper limit on the ratio of excluded cross section to the predicted
...
island at low dark matter masses for which this analysis is not expected to exclude.
Instead, the observed value of $\mu_{\mathrm{up}}$ is smaller than 1 throughout this 
...
An excess of photons in the galactic center observed in FermiLAT data has led to speculations that DM
annihilation is mediated by a light pseudoscalar~\cite{Hooper:2010mq,Gordon:2013vta,Daylan:2014rsa,Calore:2014xka,Agrawal:2014oha,Calore:2014nla}.  The
...
using data from FermiLAT, are shown as solid colour regions. Under the simplified model used, all of these
...
of the models, a minimum mediator width is assumed. For the pseudoscalar, 68\%
...
Lorentz-boosted and resolved scenarios. The sensitivity to a mono-V signature is
enhanced by categorising the events according to the origin of the jets in the
event as either a quark/gluon or from a hadronically decaying vector
boson.  The sensitivity is further increased by using
the full shape of the $\ETm$ distribution to discriminate signal from standard
model background prediction.  No significant deviation from the expected SM
background is observed in the \ETm distributions.  The results of the search
...
the first at CMS to be interpreted within the simplified models for DM
production. 
