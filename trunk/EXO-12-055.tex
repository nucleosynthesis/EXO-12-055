...
a model-independent means of exploring DM production at the LHC, while
mono-V, V=W or Z boson, searches~\cite{monolep,Aad:2014vka,Aad:2013oja,ATLAS:2014wra} target the
...
interactions occurs predominantly through gluon-fusion via a top-quark loop~(Fig. ~\ref{fig:monoXfeyn}(a)).  When EWSB is present in the model, mono-V
...
The impact of this choice is found to be up to 30-80\% in the expected signal yield in the relevant kinematic region 
...
The $\tau_2/\tau_1$ ratio is required to be < 0.5 and $m_{\mathrm{prune}}$ 
...
$\Delta\phi(\mathrm{ak5,ca8})$, is < 2 radians,~the event is selected. Events
...
least one ak5 jet within $|\eta|<2.0$ with \pt greater than 150 \gev and \ETm
...
binned likelihood fit is performed in the $\ETm$ range 250(200)-1000 \gev 
...
two isolated muons are required with opposite charge, $p_{\mathrm{T}}^{\mu_{1},\mu_{2}}>20,10$ GeV and an 
...
isolated photon with $\pt$ greater than 150 GeV~\cite{Khachatryan:2015iwa}. 
...
are denoted $B^{\gamma}$, $B^{Z}$ and $B^{W}$ in Eq.(~\ref{eqn:candclh}),
respectively.
...
\begin{sidewaystable}[htbp]
  \begin{center}
	...
	\hline
	\ETm$ bin (GeV)$   & Observed &  Total bkg & Z($\rightarrow ll$)+jets & Top & QCD & W($\rightarrow l\nu$)+jets & Dibosons &  Z($\rightarrow \nu\nu$)+jets \\
...
determining the points for which $\mu\ge1$ is excluded at 90\%
confidence level (CL) or more.
...
island at low DM masses for which this analysis is not expected to exclude.
